...
function queuePath() {
  return join5(homedir4(), ".deeplake", "notifications-queue.json");
}
function lockPath() {
...
async function withQueueLock(fn) {
  ...
  mkdirSync3(join5(homedir4(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
...
import { delimiter, join as join30 } from "node:path";
...
async function maybeEmitSessionNotifications(sessionId) {
  try {
    if (!sessionId)
      return;
    const sentinel = join31(dirname12(sessionEventCachePath(sessionId)), `.notified-${sessionId}`);
    try {
...
function resolveEmbedDaemonPath() {
  return join31(dirname12(fileURLToPath4(import.meta.url)), "embeddings", "embed-daemon.js");
}
