...
async function resolveWorkspaceOverride(creds, log19 = () => {
}, cwd = process.cwd()) {
  const found = findDirConfig(cwd);
  ...
  try {
    ...
    if (!match) {
      ...
      log19(`workspace '${ref}' not found in org ${orgId} (from ${source}${found ? `: ${found.path}` : ""})`);
      return {
    ...
    if (match.id !== ref)
      log19(`workspace '${ref}' resolved to id '${match.id}'`);
    return { creds: cached === match.id ? creds : rememberWorkspaceAlias(creds, orgId, ref, match.id) };
  } catch (e) {
    log19(`workspace resolve skipped (${e.message}); ${cached ? `using cached id '${cached}'` : "no cached id"}`);
    return { creds };
  }
}
...
var defaultSpawn = (cmd, args) => {
  ...
  const child = spawn3(needsShell ? shellFile(cmd) : cmd, args, {
    detached: true,
    stdio: "ignore",
...
function pruneOrphanedEntries(path = manifestPath()) {
  ...
  for (const e of m.entries) {
    if (existsSync8(join17(e.installRoot, e.dirName))) {
      live.push(e);
      continue;
    }
...
function resolveDetected(home) {
  const out = [];
  const codexInstalled = existsSync9(join18(home, ".codex"));
  const piInstalled = existsSync9(join18(home, ".pi", "agent"));
  const hermesInstalled = existsSync9(join18(home, ".hermes"));
...
function resolvePullDestination(install, cwd) {
  if (install === "global")
    return join19(homedir12(), ".claude", "skills");
  if (!cwd)
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    if (staleDir && staleDir !== dirName && !(opts.dryRun ?? false) && existsSync10(skillFile) && existsSync10(join19(root, staleDir))) {
      try {
...
function spawnGraphPullWorker(cwd, bundleDir, deps = {}) {
  if (process.env.HIVEMIND_GRAPH_PULL === "0")
    return;
  const workerPath = join21(bundleDir, "graph-pull-worker.js");
  const opts = {
    detached: true,
...
function statePath() {
  return join26(homedir14(), ".deeplake", "notifications-state.json");
}
function readState() {
  try {
...
function tryClaim(n) {
  const home = resolve5(homedir14());
  const claimsDir = join26(home, ".deeplake", "notifications-claims");
  try {
...
function claimPathFor(claimsDir, n) {
  ...
  return join26(claimsDir, `${safeId}-${keyHash}`);
}
...
function cacheFilePath() {
  return join27(homedir15(), ".deeplake", "hivemind-stats-cache.json");
}
...
var PROJECTS_DIR = () => join28(homedir16(), ".claude", "projects");
var STATE_FILE = () => join28(homedir16(), ".claude", ".hivemind_brief_state.json");
...
function mineLocal(cutoff) {
  ...
  for (const d of projDirs) {
    ...
    let files;
    try {
      files = readdirSync3(join28(base, d));
    } catch {
      continue;
    }
    for (const f of files) {
      ...
      if (Date.now() > deadline)
        break;
      const s = loadLocalSession(join28(base, d, f), cutoff);
      if (s)
        out.push(s);
    }
  }
...
var STATE_DIR = join29(homedir17(), ".claude", "hooks", "summary-state");
var YIELD_BUF = new Int32Array(new SharedArrayBuffer(4));
function statePath2(sessionId) {
  return join29(STATE_DIR, `${sessionId}.json`);
}
function endedMarkerPath(sessionId) {
  return join29(STATE_DIR, `${sessionId}.ended`);
}
function ownerPath(sessionId) {
  return join29(STATE_DIR, `${sessionId}.owner`);
}
...
function statsFilePath() {
  return join30(homedir18(), ".deeplake", "usage-stats.jsonl");
}
...
function countUserGeneratedSkills(userName) {
  if (!userName)
    return 0;
  const dir = join30(homedir18(), ".claude", "skills");
  if (!existsSync18(dir))
    return 0;
...
async function main() {
  ...
  if (!creds?.token) {
    ...
    const wsOverride = await resolveWorkspaceOverride(creds, log18, cwd);
