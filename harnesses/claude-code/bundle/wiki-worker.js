...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync3, existsSync as existsSync4, statSync as statSync3, utimesSync, appendFileSync as appendFileSync3, mkdirSync as mkdirSync5, rmSync } from "node:fs";
...
function buildClaudeInvocation(claudeBin, prompt, grants) {
...
var SUMMARY_BACKDATE_MS = 6e4;
function markSummaryUnwritten(path, fs) {
  if (!fs.existsSync(path))
    return { mtimeMs: 0, trusted: true };
  ...
  try {
    ...
    return { mtimeMs, trusted: Math.abs(mtimeMs - sentinel.getTime()) < 2e3 };
  } catch {
    return { mtimeMs: 0, trusted: false };
  }
}
function summaryWasWritten(path, baseline, contentChanged, fs) {
  if (!baseline.trusted)
...
async function main() {
  try {
    ...
    const summaryBaseline = markSummaryUnwritten(tmpSummary, { existsSync: existsSync4, utimesSync, statSync: statSync3 });
    try {
    ...
    if (existsSync4(tmpSummary)) {
      ...
      if (!summaryWasWritten(tmpSummary, summaryBaseline, summaryChanged, { statSync: statSync3 })) {
