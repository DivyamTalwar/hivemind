...
function findDirConfig(startDir, stopAt) {
  ...
  for (; ; ) {
    for (const name of DIR_CONFIG_FILENAMES) {
      ...
      try {
        const raw = parseDirConfig(readFileSync3(candidate, "utf-8"));
        if (raw)
...
var LOG = join4(homedir3(), ".deeplake", "hook-debug.log");
function isDebug() {
...
function log(tag, msg) {
  if (!isDebug())
    return;
  try {
    mkdirSync2(dirname2(LOG), { recursive: true });
...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync3, writeSync, mkdirSync as mkdirSync5, existsSync as existsSync2, unlinkSync as unlinkSync2, openSync, closeSync, statSync } from "node:fs";
...
import { appendFileSync as appendFileSync3, mkdirSync as mkdirSync6, readFileSync as readFileSync6, readdirSync, statSync as statSync2, unlinkSync as unlinkSync3 } from "node:fs";
import { homedir as homedir7 } from "node:os";
import { join as join10 } from "node:path";
var CACHE_DIR = join10(homedir7(), ".claude", "hooks", "session-cache");
function pruneStaleSessionEventCaches(ttlMs = 14 * 24 * 3600 * 1e3, now = Date.now()) {
  try {
    for (const name of readdirSync(CACHE_DIR)) {
      if (!name.endsWith(".jsonl"))
        continue;
      const p = join10(CACHE_DIR, name);
      try {
...
import { writeFileSync as writeFileSync4, mkdirSync as mkdirSync7, appendFileSync as appendFileSync4, chmodSync } from "node:fs";
...
function spawnSkillifyWorker(opts) {
  ...
  const tmpDir = join12(tmpdir2(), `deeplake-skillify-${projectKey}-${Date.now()}`);
...
import { readFileSync as readFileSync7, writeFileSync as writeFileSync5, writeSync as writeSync2, mkdirSync as mkdirSync8, renameSync as renameSync3, rmdirSync, existsSync as existsSync5, lstatSync, unlinkSync as unlinkSync4, openSync as openSync2, closeSync as closeSync2 } from "node:fs";
...
import { existsSync as existsSync6, mkdirSync as mkdirSync9, readFileSync as readFileSync8, writeFileSync as writeFileSync6 } from "node:fs";
...
import { appendFileSync as appendFileSync5, existsSync as existsSync8, mkdirSync as mkdirSync10, readFileSync as readFileSync10, readdirSync as readdirSync2 } from "node:fs";
import { dirname as dirname7, join as join17 } from "node:path";
import { homedir as homedir11 } from "node:os";
var log3 = (msg) => log("usage-tracker", msg);
function statsFilePath() {
  return join17(homedir11(), ".deeplake", "usage-stats.jsonl");
}
...
function isHivemindPluginEnabled() {
  try {
    const settingsPath = join18(homedir12(), ".claude", "settings.json");
