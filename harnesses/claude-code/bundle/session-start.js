...
var WORKSPACE_LOOKUP_TIMEOUT_MS = 5e3;
function sanitizeForContext(value, max = 64) {
  const flat = value.replace(/[\p{Cc}\p{Cf}\s]+/gu, " ").trim();
  return flat.length > max ? `${flat.slice(0, max - 1)}\u2026` : flat;
}
async function resolveWorkspaceOverride(creds, log8 = () => {
}, cwd = process.cwd()) {
  const found = findDirConfig(cwd);
  const ref = process.env.HIVEMIND_WORKSPACE_ID ?? found?.raw.workspaceId;
  const token = process.env.HIVEMIND_TOKEN ?? creds.token;
  if (!ref || ref === "default" || !token)
    return { creds };
  ...
  try {
    ...
    if (!match) {
      ...
      log8(`workspace '${ref}' not found in org ${orgId} (from ${source}${found ? `: ${found.path}` : ""})`);
      return {
        creds: cached ? forgetWorkspaceAlias(creds, orgId, ref) : creds,
        warning: `Workspace '${sanitizeForContext(ref)}' (from ${source}) does not match any workspace in this org (available: ${names}); capture and memory search will fail until it is fixed. Prefer \`hivemind workspace switch <name>\` over the env var.`
      };
    }
    if (match.id !== ref)
      log8(`workspace '${ref}' resolved to id '${match.id}'`);
    return { creds: cached === match.id ? creds : rememberWorkspaceAlias(creds, orgId, ref, match.id) };
  } catch (e) {
    log8(`workspace resolve skipped (${e.message}); ${cached ? `using cached id '${cached}'` : "no cached id"}`);
    return { creds };
  }
}
function rememberWorkspaceAlias(creds, orgId, ref, id) {
...
function forgetWorkspaceAlias(creds, orgId, ref) {
  const latest = loadCredentials() ?? creds;
...
var defaultSpawn = (cmd, args) => {
  ...
  const child = spawn2(needsShell ? shellFile(cmd) : cmd, args, {
    detached: true,
    stdio: "ignore",
  ...
  child.unref();
  child.on("error", (err) => {
    log4(`autoupdate spawn failed: ${err.message}`);
  });
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join15(homedir9(), ".deeplake", "state", "skillify");
}
...
function migrateLegacyStateDir() {
  ...
  const legacy = join16(dirname5(current), "skilify");
...
function pruneOrphanedEntries(path = manifestPath()) {
  ...
  for (const e of m.entries) {
    if (existsSync6(join17(e.installRoot, e.dirName))) {
      live.push(e);
      continue;
    }
...
function resolveDetected(home) {
  const out = [];
  const codexInstalled = existsSync7(join18(home, ".codex"));
  const piInstalled = existsSync7(join18(home, ".pi", "agent"));
  const hermesInstalled = existsSync7(join18(home, ".hermes"));
...
function resolvePullDestination(install, cwd) {
  if (install === "global")
    return join19(homedir11(), ".claude", "skills");
  if (!cwd)
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    const staleFile = staleOwned ? join19(root, staleDir, "SKILL.md") : null;
    ...
    if (staleDir && staleDir !== dirName && !(opts.dryRun ?? false) && existsSync8(skillFile) && existsSync8(join19(root, staleDir))) {
      try {
        ...
        if (canonicalRecorded && staleEntry) {
          rmSync(join19(root, staleDir), { recursive: true, force: true });
...
function retireEntry(entry) {
  ...
  rmSync2(join20(entry.installRoot, entry.dirName), { recursive: true, force: true });
  removePullEntry(entry.install, entry.installRoot, entry.dirName);
}
function migrateEntry(entry) {
  if (entry.install !== "global")
    return false;
  const staleDir = join20(entry.installRoot, entry.dirName);
  const staleFile = join20(staleDir, "SKILL.md");
  ...
  const cappedDirPath = join20(entry.installRoot, cappedDir);
  const cappedFile = join20(cappedDirPath, "SKILL.md");
...
var LOCAL_MANIFEST_PATH = join21(homedir12(), ".claude", "hivemind", "local-mined.json");
var LOCAL_MINE_LOCK_PATH = join21(homedir12(), ".claude", "hivemind", "local-mined.lock");
...
var HOME = homedir13();
var HIVEMIND_DIR = join22(HOME, ".claude", "hivemind");
var LOG_PATH = join22(HOME, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join22(HOME, ".claude", "projects");
var LOCK_STALE_MS2 = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname9(fileURLToPath(import.meta.url));
    const cliPath = join22(thisDir, "..", "..", "bundle", "cli.js");
...
function findHivemindLauncher() {
  ...
  try {
    ...
    const out = execFileSync2(lookup, ["hivemind"], {
      encoding: "utf-8",
      stdio: ["ignore", "pipe", "ignore"],
...
function hasLocalClaudeSessions() {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync2(join22(CLAUDE_PROJECTS_DIR, sub));
    } catch {
      continue;
    }
...
function maybeAutoMineLocal(opts = {}) {
  ...
  try {
    mkdirSync12(join22(HOME, ".claude", "hooks"), { recursive: true });
...
function lastBuildPath(baseDir, worktreeId) {
  if (worktreeId !== void 0) {
    return join23(baseDir, "worktrees", worktreeId, ".last-build.json");
  }
  return join23(baseDir, ".last-build.json");
}
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join25(homedir14(), ".hivemind", "graphs");
}
function repoDir(repoKey) {
  return join25(graphsRoot(), repoKey);
}
...
function graphContextLine(cwd, deps = {}) {
  ...
  try {
    ...
    baseDir = repoDir(key);
    snapshotsDir = join26(baseDir, "snapshots");
  } catch {
    return null;
  }
  ...
  const snapshotPath = join26(snapshotsDir, `${snapshotFile}.json`);
...
function spawnGraphPullWorker(cwd, bundleDir, deps = {}) {
  if (process.env.HIVEMIND_GRAPH_PULL === "0")
    return;
  const workerPath = join27(bundleDir, "graph-pull-worker.js");
  const opts = {
    detached: true,
    stdio: ["ignore", "ignore", "ignore"]
  };
...
var STATE_DIR = join28(homedir15(), ".claude", "hooks", "summary-state");
...
var HOME2 = homedir16();
var { log: wikiLog } = makeWikiLogger(join29(HOME2, ".claude", "hooks"));
...
async function main() {
  ...
  if (!creds?.token) {
    ...
    const wsOverride = await resolveWorkspaceOverride(creds, log7, input.cwd ?? process.cwd());
