...
import { existsSync as existsSync3, mkdirSync as mkdirSync7, readFileSync as readFileSync8, writeFileSync as writeFileSync6 } from "node:fs";
...
import { closeSync as closeSync2, mkdirSync as mkdirSync5, openSync as openSync2, readFileSync as readFileSync6, unlinkSync as unlinkSync3, writeFileSync as writeFileSync4 } from "node:fs";
import { createHash } from "node:crypto";
import { join as join8, resolve as resolve4 } from "node:path";
import { homedir as homedir6 } from "node:os";
var log3 = (msg) => log("notifications-state", msg);
function statePath() {
  return join8(homedir6(), ".deeplake", "notifications-state.json");
}
function readState() {
  try {
...
function tryClaim(n) {
  const home = resolve4(homedir6());
  const claimsDir = join8(home, ".deeplake", "notifications-claims");
  try {
...
function claimPathFor(claimsDir, n) {
  ...
  return join8(claimsDir, `${safeId}-${keyHash}`);
}
...
function cacheFilePath() {
  return join9(homedir7(), ".deeplake", "hivemind-stats-cache.json");
}
...
function readCache(scopeKey) {
  if (!existsSync2(cacheFilePath()))
    return {};
  try {
    const parsed = JSON.parse(readFileSync7(cacheFilePath(), "utf-8"));
...
function writeCache(scopeKey, data) {
  try {
    mkdirSync6(dirname3(cacheFilePath()), { recursive: true });
...
import { existsSync as existsSync4, readdirSync, statSync as statSync3, writeFileSync as writeFileSync7, readFileSync as readFileSync9, openSync as openSync3, readSync, closeSync as closeSync3 } from "node:fs";
...
var PROJECTS_DIR = () => join11(homedir8(), ".claude", "projects");
var STATE_FILE = () => join11(homedir8(), ".claude", ".hivemind_brief_state.json");
...
function hasState() {
  return existsSync4(STATE_FILE());
}
function lastBriefMs() {
  try {
    if (!existsSync4(STATE_FILE()))
      return null;
    const raw = JSON.parse(readFileSync9(STATE_FILE(), "utf-8"));
...
function mineLocal(cutoff) {
  ...
  for (const d of projDirs) {
    ...
    let files;
    try {
      files = readdirSync(join11(base, d));
    } catch {
      continue;
    }
    for (const f of files) {
      ...
      if (Date.now() > deadline)
        break;
      const s = loadLocalSession(join11(base, d, f), cutoff);
      if (s)
        out.push(s);
    }
  }
...
import { readFileSync as readFileSync10, writeFileSync as writeFileSync8, writeSync, mkdirSync as mkdirSync8, existsSync as existsSync5, unlinkSync as unlinkSync4, openSync as openSync4, closeSync as closeSync4, statSync as statSync4 } from "node:fs";
import { homedir as homedir9 } from "node:os";
import { join as join12 } from "node:path";
var STATE_DIR = join12(homedir9(), ".claude", "hooks", "summary-state");
var YIELD_BUF = new Int32Array(new SharedArrayBuffer(4));
function statePath2(sessionId) {
  return join12(STATE_DIR, `${sessionId}.json`);
}
function endedMarkerPath(sessionId) {
  return join12(STATE_DIR, `${sessionId}.ended`);
}
function ownerPath(sessionId) {
  return join12(STATE_DIR, `${sessionId}.owner`);
}
function procInfo(pid) {
  try {
    const s = readFileSync10(`/proc/${pid}/stat`, "utf-8");
...
function readOwner(sessionId) {
  try {
    return JSON.parse(readFileSync10(ownerPath(sessionId), "utf-8"));
  } catch {
    return null;
  }
}
...
function isSessionLive(sessionId, withinMs = activeWindowMs()) {
  ...
  if (owner === "dead")
    return false;
  try {
    const mtimeMs = statSync4(statePath2(sessionId)).mtimeMs;
...
import { appendFileSync as appendFileSync2, existsSync as existsSync6, mkdirSync as mkdirSync9, readFileSync as readFileSync11, readdirSync as readdirSync2 } from "node:fs";
import { dirname as dirname4, join as join13 } from "node:path";
import { homedir as homedir10 } from "node:os";
var log10 = (msg) => log("usage-tracker", msg);
function statsFilePath() {
  return join13(homedir10(), ".deeplake", "usage-stats.jsonl");
}
...
function countUserGeneratedSkills(userName) {
  if (!userName)
    return 0;
  const dir = join13(homedir10(), ".claude", "skills");
  if (!existsSync6(dir))
    return 0;
...
// dist/src/notifications/rules/embeddings-nudge.js
var embeddingsNudgeRule = {
  id: "embeddings-nudge",
  trigger: "session_start",
  evaluate({ embeddingsStatus: embeddingsStatus2 }) {
    if (embeddingsStatus2 === void 0 || embeddingsStatus2 === "enabled")
      return null;
    return {
      id: "embeddings-nudge",
      severity: "warn",
...
import { existsSync as existsSync7, mkdirSync as mkdirSync10, readFileSync as readFileSync12, renameSync as renameSync2, writeFileSync as writeFileSync9 } from "node:fs";
