...
function registryPath() {
  return process.env.HIVEMIND_DOCS_AUTO_FILE ?? join3(homedir3(), ".deeplake", "docs-auto.json");
}
...
var CLAUDE_BASE_FLAGS = ["--no-session-persistence", "--model", "haiku"];
var CLAUDE_BYPASS_FLAGS = ["--permission-mode", "bypassPermissions"];
function permissionFlags(grants, quotePaths = false) {
  if (!grants)
    return [...CLAUDE_BYPASS_FLAGS];
  const flags = [];
  for (const dir of grants.addDirs ?? [])
    flags.push("--add-dir", quotePaths ? `"${dir}"` : dir);
  if (grants.allowedTools?.length)
    flags.push("--allowedTools", ...grants.allowedTools);
...
function buildClaudeStdinInvocation(claudeBin, prompt, grants) {
  return buildStdinPromptInvocation(claudeBin, ["-p", ...CLAUDE_BASE_FLAGS, ...permissionFlags(grants)], prompt);
}

// dist/src/user-config.js
import { existsSync as existsSync2, mkdirSync as mkdirSync3, readFileSync as readFileSync6, renameSync as renameSync3, writeFileSync as writeFileSync3 } from "node:fs";
...
import { chmodSync, existsSync as existsSync3, mkdirSync as mkdirSync4, readFileSync as readFileSync7, unlinkSync as unlinkSync2, writeFileSync as writeFileSync4 } from "node:fs";
...
function installPostCommitHook(cwd, opts = {}) {
  ...
  writeFileSync4(path, buildHookFile(hivemindPath), { mode: 493 });
  try {
    chmodSync(path, 493);
  } catch {
  }
...
function loadCurrentSnapshot(cwd) {
  ...
  const snapPath = join9(baseDir, "snapshots", `${fileBase}.json`);
  if (!existsSync4(snapPath))
    return null;
  try {
    const snap = JSON.parse(readFileSync8(snapPath, "utf8"));
...
// dist/src/commands/graph.js
import { readFileSync as readFileSync16, readdirSync as readdirSync2 } from "node:fs";
...
import { existsSync as existsSync5, mkdirSync as mkdirSync5, readFileSync as readFileSync9, readdirSync, writeFileSync as writeFileSync5, cpSync, rmSync, symlinkSync, unlinkSync as unlinkSync3, lstatSync } from "node:fs";
...
import { existsSync as existsSync6, mkdirSync as mkdirSync6, readFileSync as readFileSync11, renameSync as renameSync4, writeFileSync as writeFileSync6 } from "node:fs";
...
import { dirname as dirname6, join as join13, resolve as resolve2 } from "node:path";
...
var LOG = join14(homedir7(), ".deeplake", "hook-debug.log");
function isDebug() {
...
import { readFileSync as readFileSync13, writeFileSync as writeFileSync7, mkdirSync as mkdirSync8, openSync, closeSync, unlinkSync as unlinkSync4, statSync as statSync2 } from "node:fs";
...
function queuePath() {
  return join15(homedir8(), ".deeplake", "notifications-queue.json");
}
function lockPath() {
...
function writeQueue(q) {
  ...
  const home = resolve4(homedir8());
  if (!_isQueuePathInsideHome(path, home)) {
    throw new Error(`notifications-queue write blocked: ${path} is outside ${home}`);
  }
  mkdirSync8(join15(home, ".deeplake"), { recursive: true, mode: 448 });
...
async function withQueueLock(fn) {
  const path = lockPath();
  mkdirSync8(join15(homedir8(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
