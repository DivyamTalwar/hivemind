...
var defaultSpawn = (cmd, args) => {
  ...
  child.unref();
  child.on("error", (err) => {
    log5(`autoupdate spawn failed: ${err.message}`);
  });
...
var { log: wikiLog } = makeWikiLogger(join15(homedir10(), ".claude", "hooks"));
async function main() {
  if (process.env.HIVEMIND_WIKI_WORKER === "1")
    return;
  const input = await readStdin();
  spawnDetachedNodeWorker(join15(__bundleDir, "graph-deps-worker.js"));
  ...
  if (embeddingsDisabled()) {
    ...
    try {
      const daemonEntry = join15(__bundleDir, "embeddings", "embed-daemon.js");
