  ...
  switch (agent) {
    ...
    case "claude_code":
      return firstExistingPath([
        join4(home, ".claude", "local", "claude"),
    ...
    case "cursor":
      return firstExistingPath([
        ...
        join4(home, ".npm-global", "bin", "cursor-agent"),
        join4(home, ".local", "bin", "cursor-agent"),
    ...
    case "hermes":
      return firstExistingPath([
        ...
        "/usr/bin/hermes",
        join4(home, ".npm-global", "bin", "hermes"),
        "/opt/homebrew/bin/hermes"
      ]) ?? join4(home, ".local", "bin", "hermes");
    case "pi":
      return firstExistingPath([
        join4(home, ".local", "bin", "pi"),
        "/usr/local/bin/pi",
        "/usr/bin/pi",
        join4(home, ".npm-global", "bin", "pi"),
        "/opt/homebrew/bin/pi"
      ]) ?? join4(home, ".local", "bin", "pi");
  }
}
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join5(homedir5(), ".deeplake", "state", "skillify");
}
...
function migrateLegacyStateDir() {
  ...
  const legacy = join6(dirname2(current), "skilify");
...
function statePath(projectKey) {
  return join7(getStateDir(), `${projectKey}.json`);
}
function lockPath(projectKey) {
  return join7(getStateDir(), `${projectKey}.lock`);
}
...
var verdictPath = join8(tmpDir, "verdict.json");
var promptPath = join8(tmpDir, "prompt.txt");
...
async function main() {
  try {
    ...
    try {
      writeFileSync3(join8(tmpDir, "gate-stdout.txt"), gate.stdout);
      if (gate.stderr)
        writeFileSync3(join8(tmpDir, "gate-stderr.txt"), gate.stderr);
    } catch {
    }
    if (gate.errored) {
