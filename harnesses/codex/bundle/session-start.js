...
async function resolveWorkspaceOverride(creds, log18 = () => {
}, cwd = process.cwd()) {
  const found = findDirConfig(cwd);
  ...
  try {
    ...
    if (!match) {
      ...
      log18(`workspace '${ref}' not found in org ${orgId} (from ${source}${found ? `: ${found.path}` : ""})`);
      return {
    ...
    if (match.id !== ref)
      log18(`workspace '${ref}' resolved to id '${match.id}'`);
    return { creds: cached === match.id ? creds : rememberWorkspaceAlias(creds, orgId, ref, match.id) };
  } catch (e) {
    log18(`workspace resolve skipped (${e.message}); ${cached ? `using cached id '${cached}'` : "no cached id"}`);
    return { creds };
  }
}
...
var LOCAL_MANIFEST_PATH = join6(homedir4(), ".claude", "hivemind", "local-mined.json");
var LOCAL_MINE_LOCK_PATH = join6(homedir4(), ".claude", "hivemind", "local-mined.lock");
...
import { closeSync, existsSync as existsSync3, mkdirSync as mkdirSync4, openSync, readdirSync, statSync as statSync2, unlinkSync as unlinkSync2 } from "node:fs";
...
function findBundledCliPath() {
  try {
    ...
    return existsSync3(cliPath) ? cliPath : null;
  } catch {
    return null;
  }
}
...
function hasLocalClaudeSessions() {
  if (!existsSync3(CLAUDE_PROJECTS_DIR))
    return false;
  let subdirs;
  try {
...
function maybeAutoMineLocal(opts = {}) {
  if (existsSync3(LOCAL_MANIFEST_PATH))
    return { triggered: false, reason: "manifest-exists" };
  if (existsSync3(LOCAL_MINE_LOCK_PATH)) {
    let stale = false;
    try {
  ...
  try {
    mkdirSync4(join7(HOME, ".claude", "hooks"), { recursive: true });
...
var LOG = join8(homedir6(), ".deeplake", "hook-debug.log");
function isDebug() {
...
function log(tag, msg) {
  if (!isDebug())
    return;
  try {
    mkdirSync5(dirname4(LOG), { recursive: true });
...
async function main() {
  ...
  if (!creds?.token) {
    ...
    const wsOverride = await resolveWorkspaceOverride(creds, log17, input.cwd ?? process.cwd());
...
Hivemind v${current}`;
  ...
  const additionalContext = creds?.token ? `Hivemind: logged in as org ${creds.orgName ?? creds.orgId} (workspace: ${creds.workspaceId ?? "default"}).${workspaceWarning}${versionNotice}` : `Hivemind: not logged in. Run \`hivemind login\` to enable shared memory + skill sharing.${versionNotice}`;
