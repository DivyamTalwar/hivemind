...
function dropUnreplaceable(src, dst) {
  ...
  for (const entry of readdirSync(src, { withFileTypes: true })) {
    ...
    if (st.isDirectory() === entry.isDirectory()) {
      if (entry.isDirectory())
        removed.push(...dropUnreplaceable(join(src, entry.name), target));
      continue;
    }
...
function pruneToSource(src, dst) {
  ...
  for (const entry of shipped) {
    ...
    if (isLink(sub) || !lstatSync(sub).isDirectory())
      continue;
    removed.push(...pruneToSource(join(src, entry.name), sub));
  }
  return removed;
}
function reportPruned(label, removed) {
  for (const path of removed)
    log(`  ${label.padEnd(15)}removed stale ${path}`);
}
...
function log2(tag, msg) {
  if (!isDebug())
    return;
  try {
    mkdirSync2(dirname3(LOG), { recursive: true });
...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync2, mkdirSync as mkdirSync3, openSync, closeSync, unlinkSync as unlinkSync2, statSync } from "node:fs";
...
async function withQueueLock(fn) {
  const path = lockPath();
  mkdirSync3(join6(homedir3(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
