...
function installCodex() {
  ...
  reportPruned("Codex", syncDir(srcBundle, join2(PLUGIN_DIR, "bundle")));
  if (existsSync2(srcSkills))
    reportPruned("Codex", syncDir(srcSkills, join2(PLUGIN_DIR, "skills")));
  tryEnableCodexHooks();
...
function installOpenclaw() {
  ...
  reportPruned("OpenClaw", syncDir(srcDist, join4(PLUGIN_DIR2, "dist")));
  ...
  if (existsSync4(srcSkills))
    reportPruned("OpenClaw", syncDir(srcSkills, join4(PLUGIN_DIR2, "skills")));
...
function installCursor() {
  ...
  reportPruned("Cursor", syncDir(srcBundle, join5(PLUGIN_DIR3, "bundle")));
...
import { existsSync as existsSync7, lstatSync as lstatSync4, readdirSync, writeFileSync as writeFileSync4, readFileSync as readFileSync4, rmSync as rmSync4, unlinkSync as unlinkSync3 } from "node:fs";
...
var SKILLS_DIR = join7(SKILLS_ROOT, "hivemind-memory");
...
function packagedSkillsSrc() {
  return join7(pkgRoot(), "harnesses", "hermes", "skills");
}
...
function installHermes() {
  ...
  for (const name of packagedSkillNames()) {
    reportPruned("Hermes", syncDir(join7(packagedSkillsSrc(), name), join7(SKILLS_ROOT, name)));
    log(`  Hermes         skill installed -> ${join7(SKILLS_ROOT, name)}`);
  }
...
function uninstallHermes() {
  for (const dir of [SKILLS_DIR, ...packagedSkillNames().map((n) => join7(SKILLS_ROOT, n))]) {
    if (!existsSync7(dir) || isLink(dir))
      continue;
...
function installPi() {
  ...
  const srcWorkers = join9(pkgRoot(), "harnesses", "pi", "bundle");
  const workersInstalled = existsSync9(srcWorkers);
  if (workersInstalled) {
...
import { copyFileSync as copyFileSync3, chmodSync, existsSync as existsSync11, lstatSync as lstatSync5, readdirSync as readdirSync2, readFileSync as readFileSync8, readlinkSync, rmSync as rmSync7, statSync as statSync2, unlinkSync as unlinkSync5 } from "node:fs";
...
async function apiGet(path, token, apiUrl, orgId, signal) {
  const headers = {
...
function findWorkspace(wsList, ref) {
  const lc = ref.toLowerCase();
  return wsList.find((w) => w.id === ref) ?? wsList.find((w) => w.name && w.name.toLowerCase() === lc);
}
async function listWorkspaces(token, apiUrl = DEFAULT_API_URL, orgId, signal) {
  const raw = await apiGet("/workspaces", token, apiUrl, orgId, signal);
...
import { existsSync as existsSync17, readFileSync as readFileSync13, readdirSync as readdirSync4, statSync as statSync4 } from "node:fs";
...
import { appendFileSync, existsSync as existsSync14, mkdirSync as mkdirSync4, readFileSync as readFileSync11, readdirSync as readdirSync3 } from "node:fs";
...
import { readdirSync as readdirSync8, existsSync as existsSync30, readFileSync as readFileSync22, mkdirSync as mkdirSync13, renameSync as renameSync6 } from "node:fs";
...
import { existsSync as existsSync19, mkdirSync as mkdirSync8, readFileSync as readFileSync15, readdirSync as readdirSync5, statSync as statSync5, writeFileSync as writeFileSync13 } from "node:fs";
...
import { existsSync as existsSync24, readdirSync as readdirSync6, rmSync as rmSync9, statSync as statSync6 } from "node:fs";
...
import { readdirSync as readdirSync7, readFileSync as readFileSync19, existsSync as existsSync25, statSync as statSync7 } from "node:fs";
...
wiki: ${planned.length - pending.length}/${planned.length} pages generated \u2014 pending: ${pending.map((g) => wikiDocId(g.key)).join(", ")}`);
  ...
  if (sub === "sync") {
    ...
    if (!loadCurrentSnapshot(cwd)) {
      ...
      const { runBuildCommand } = await import("./graph-LYJA47KW.js");
...
function runClaude2(claudeBin, prompt, timeoutMs, grants) {
...
async function stageSession(input, opts) {
  ...
  const ran = await runAgent(opts.claudeBin, prompt, opts.timeoutMs, {
    addDirs: [dirname16(input.jsonlPath), stagingDir],
...
import { closeSync as closeSync3, existsSync as existsSync40, mkdirSync as mkdirSync20, openSync as openSync3, readdirSync as readdirSync9, statSync as statSync8, unlinkSync as unlinkSync13 } from "node:fs";
...
function hasLocalClaudeSessions() {
  ...
  let subdirs;
  try {
    subdirs = readdirSync9(CLAUDE_PROJECTS_DIR);
  } catch {
    return false;
  }
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync9(join45(CLAUDE_PROJECTS_DIR, sub));
    } catch {
      continue;
    }
...
import { dirname as dirname18, join as join47, sep as sep2 } from "node:path";
...
var defaultSpawn = (cmd, args) => {
  ...
  execFileSync7(needsShell ? shellFile(bin) : bin, args, {
    stdio: "inherit",
    shell: needsShell
  });
};
function removeNpmPowerShellShim(deps = {}) {
  ...
  if (!binDir) {
    ...
    if (!resolved)
      return false;
    binDir = dirname18(resolved);
  }
  const ps1 = join47(binDir, "hivemind.ps1");
  const cmd = join47(binDir, "hivemind.cmd");
  if (!existsSync42(ps1))
    return false;
  if (!existsSync42(cmd))
    return false;
  let content = "";
  try {
    content = readFileSync32(ps1, "utf-8");
  } catch {
    return false;
  }
  const CMD_SHIM_PREAMBLE = "$basedir=Split-Path $MyInvocation.MyCommand.Definition -Parent";
  if (!content.includes(CMD_SHIM_PREAMBLE))
    return false;
  if (!content.includes("node_modules/@deeplake/hivemind") && !content.includes("node_modules\\@deeplake\\hivemind"))
    return false;
  try {
    unlinkSync15(ps1);
    return true;
...
async function runInstallAll(args) {
  ...
  const { loadConfig: loadConfig2 } = await import("./config-GP5RBDGR.js");
  ...
  await runInstallDocsOnboarding({
    ...
    onboard: async ({ root, orgId, orgName }) => {
      const { runDocsOnboarding: runDocsOnboarding2 } = await import("./onboarding-IUC5PNOE.js");
...
async function main() {
  ...
  if (cmd === "graph") {
    ...
    let runGraphCommand;
    try {
      ({ runGraphCommand } = await import("./graph-LYJA47KW.js"));
    } catch (err) {
